...
chatgpt_output<-read_excel("H:/Shared drives/2024 FIRE-SA/Fall Course/tickers.xlsx")
...
chat.name <-stri_encode(chat$company, "","UTF-8")
...
for (i in 2:100) {
  ...
  output<-as.data.frame(cbind(flight.name[i,1], chat.name[best_fit], round(similarity,3)))
